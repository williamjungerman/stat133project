...
#write.csv("~/Downloads/meteorite-landings.csv", x = meteorite)
```

Add in reclat and reclong
```{r}
meteorite <- meteorite %>%
  extractMatches("([-+]?[0-9]*\\.[0-9]+|[0-9]+)", GeoLocation, reclat=1) %>%
  extractMatches("([-+]?[0-9]*\\.[0-9]+|[0-9]+)\\)$", GeoLocation, reclong=1)

meteorite$reclat = as.numeric(as.character(meteorite$reclat))
meteorite$reclong = as.numeric(as.character(meteorite$reclong))
```
...
```{r}
coords <- data.frame('Long' = meteorite$reclat, 'Lat' = meteorite$reclat)
